...
// ── golden roll parity ─────────────────────────────────────────────────────
// The seeded golden-problem coin flip must land identically in both ports:
// same child + same lifetime attempt index -> same draw, byte for byte.
  ...
  for (const childId of children) {
    for (let attemptIndex = 0; attemptIndex < 40; attemptIndex++) {
      out.push({
        childId,
        attemptIndex,
        draw: goldenDraw(childId, attemptIndex),
        goldenAtRate: isGoldenEncounter(childId, attemptIndex, rate),
      });
    }
  }
  return out;
...
console.log(`  elo=${fixtures.elo.length} problemELO=${fixtures.problemELO.length} replayKeys=${fixtures.replayKeys.length} learner=${fixtures.learner.length} goldenRolls=${fixtures.goldenRolls.length}`);
