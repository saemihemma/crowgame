...
Game:
- `godot/data/registries/scenes.json` registers 8 scenes: `boot`, `cinematic`, `login`, `main_menu`, `level_select`, `game`, `cloud_panel`, `parent_report`
...
- **52** `.gd` scripts under `godot/scripts/`, **25** `.tscn` scenes under `godot/scenes/`
...
- `godot/data/i18n/strings_en.json`: **301** keys, matched key-for-key by
  `strings_is.json`
...
- `crow_prologue_seen` (the opening film is a first-*launch* event, so it is
  stored per device rather than per child; clear it to watch the prologue again)
