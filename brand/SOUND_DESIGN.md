...
**Generated by API, a family at a time.** This is the normal path, not a
fallback:

```bash
npm run audio:gen -- --check                  # key + network, spends nothing
npm run audio:gen -- --family BODY  --takes 3 # 18 requests
npm run audio:gen -- --family WORLD --takes 3 # 45
npm run audio:gen -- --family VOICE --takes 3 # 93
npm run audio:gen -- --all --takes 3 --yes    # 174, if you would rather
```

ElevenLabs rate-limits aggressively and every request is metered, so the runner
is built for that rather than against it:

| | |
| --- | --- |
| **Staggered** | a shared minimum gap between request *starts* (`--gap-ms`, 600). Concurrency alone does not help — four workers with no spacing still fire four requests in the same millisecond, and a limiter counts arrivals. |
| **Adaptive** | every 429 widens the gap; ten clean requests narrow it. A long batch converges on the pace the account is actually allowed instead of a number guessed here. |
| **Polite** | `Retry-After` is honoured when the response carries it, exponential backoff with jitter when it does not. Jitter matters: without it every backed-off worker wakes together and trips the limit again. |
| **Frugal** | a 401 or 422 fails immediately — retrying a bad key just spends the budget faster on the same mistake — and five failures in a row stop the whole run. |
| **Capped** | more than `--max-requests` (40) needs `--yes`. `--all --takes 5` is 290 generations and reads almost exactly like `--key x --takes 5`. |
| **Resumable** | a take already on disk is skipped, so re-running after a dropped connection retries only the gaps. `--force` regenerates. |

Two at a time by default; `--concurrency N` changes it.
