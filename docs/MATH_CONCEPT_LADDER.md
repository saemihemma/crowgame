...
| --- | --- | --- | --- |
| `addition.missing_part` | 2-46 | `missing_addend` | The unknown is a PART, not the result. Count up inside the whole to find it. |
| `addition.balance` | 2-46 | `relational_equals` | The whole can be written first. `=` means both sides are the same amount. |
| `subtraction.missing_part` | 3-46 | `missing_subtrahend` | How many went. Count back to the amount that is left, and the hops are the answer. |
| `subtraction.start_unknown` | 6-46 | `missing_minuend` | How many there were to begin with. Put the part that went back on -- the hardest CGI tier. |
| `subtraction.balance` | 4-46 | `subtraction_relational` | The answer written first: `5 = 12 - ?`. |
